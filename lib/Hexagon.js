  ...
  Hexagon.prototype.draw = function() {
    ...
    var c
      ...
      /* 6_wht  */ new THREE.Color().setRGB( 0.60, 0.60, 0.80 ),
    ...
    this.meshTop = geometry
    return geometry
  }

  Hexagon.prototype.drawSkirts = function() {
    var numVerts
    ...
    if (this.hasSkirts) {
      ...
      for (var i=1; i<=6; i++) {
        ...
        if (i > 1) {
          ...
          geometry.vertices.push(this.meshTop.vertices[i].clone())
          var j = this.meshTop.vertices[i].clone()
          j.y = 0
          geometry.vertices.push(j)
          numVerts += 2

          var skirt_t1 = new THREE.Face3(i*2-2, i*2-4, numVerts-1)
            , skirt_t2 = new THREE.Face3(i*2-3, numVerts-1, i*2-4)
          skirt_t1.color = skirt_t2.color = color

          geometry.faces.push(skirt_t1, skirt_t2)

        } else {
          // add the first pair of vertices.
          geometry.vertices.push(this.meshTop.vertices[i].clone())
          var j = this.meshTop.vertices[i].clone()
          j.y = 0
          geometry.vertices.push(j)
      ...
      // add the last skirt.
      var skirt_t1 = new THREE.Face3(0,  10, 1)
        , skirt_t2 = new THREE.Face3(10, 11, 1)
      skirt_t1.color = skirt_t2.color = color
  ...
  Hexagon.prototype.get_neighbors = function(southEast) {
    // in a pointy-top axial grid this returns the coordinates
    // of the connected cells, with an option to select only
    // cells connected at the South & East edges.
    var _se = southEast || 0
      , neighbors
      , r = []
