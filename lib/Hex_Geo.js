          ...
          for (var i=0; i<_n.length; i++) {
            ...
            if (i>2) {
              ...
              if (a1.length === 2 && a2.length === 2) {
                ...
                g.vertices[i1+a1[0]].copy(g.vertices[i2+a2[0]])
                g.vertices[i2+a2[1]].copy(g.vertices[i1+a1[1]])
                */
                    ...
                    for (var j=1; j<m; j++) {
                      for (var k=1; k<m; k++) {
                        if (g3.vertices[n].distanceTo(g1.vertices[k]) < 20 &&
                            ...
                            vc.set(0, 0, 0)
                            vc.add(g.vertices[i1+k])
                            vc.add(g.vertices[i2+j])
                            vc.add(g.vertices[i3+n])
                            vc.divideScalar(3)

                            // merge vertices in main geometry.
                            g.vertices[i1+k].set(vc)
                            g.vertices[i2+j].set(vc)
                            g.vertices[i3+n].set(vc)

                            // adjust vertices in each hexagon geometry.
                            g1.vertices[k].set(vc)
                            g2.vertices[j].set(vc)
                            g3.vertices[n].set(vc)
                            */
                        }
                      }
                    }
